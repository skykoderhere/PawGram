...
Yes. PawGram is built as a clone. We manually modify the package name (`com.pawgram.android`), which isolates its data and installation from the official app. You can safely run both on the same device.
...
PawGram unlocks Instagram's Developer Options by default. To easily enable features, you can drop our readymade `mc_overrides.json` file into `/data/data/com.pawgram.android/files/metaconfig/`. Please refer to `dev/ConfigApplyGuide.md` for full instructions.
